...
class Main_code(object):
    """
    Main_code class integrates YOLOv5 for license plate detection and an OCR model
    for reading the detected plates. It processes the image, detects license plates,
    and uses the OCR model to extract text from those plates.
    """
    ...
    def __call__(self, image):
        ...
        for box in boxes:
            ...
            x1, y1, x2, y2 = (
                max(0, int((x1 - (640 // 2 - w2 // 2)) / fx)),
                max(0, int((y1 - (640 // 2 - h2 // 2)) / fx)),
                min(w, int((x2 - (640 // 2 - w2 // 2)) / fx)),
                min(h, int((y2 - (640 // 2 - h2 // 2)) / fx)),
            )
...
def DrawChinese(img, text, positive, fontSize=20, fontColor=(255, 0, 0)):
    """
    Function to draw Chinese text on an image at a specific position.
    """
    cv2img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)  # Convert BGR to RGB
    pilimg = Image.fromarray(cv2img)
    # Draw Chinese characters on the image using PIL
    draw = ImageDraw.Draw(pilimg)  # Draw on the image
    font = ImageFont.truetype("MSJHL.TTC", fontSize, encoding="utf-8")  # Font settings
    draw.text(positive, text, fontColor, font=font)  # Draw the text at the position
        ...
        for box in boxes:
            ...
            if c == 2 or c == 5:
                ...
                if result:
                    ...
                    plates.append(
                        (x1, y1, x2, y2, plate, x11 + x1, y11 + y1, x22 + x1, y22 + y1)
                    )
        for plate in plates:
            ...
            image = cv2.rectangle(
                image, (x11 - 5, y11 - 5), (x22 + 5, y22 + 5), (0, 0, 255), 2
            )
