...
class Trainer:
    ...
    def __init__(self):
        ...
        if os.path.exists(config.weight):
            ...
            print("Successfully loaded network parameters")
        else:
            print("Network parameters not loaded")
    ...
    def count_loss(self, predict, target):
        ...
        if n > 0:
            loss_c_positive = self.c_loss(
                predict_positive[:, 0:2], target_positive[:, 0].long()
            )
        else:
            loss_c_positive = 0
        loss_c_nagative = self.c_loss(
            predict_negative[:, 0:2], target_negative[:, 0].long()
        )
