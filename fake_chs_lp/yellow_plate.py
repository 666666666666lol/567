...
class Draw:
    _font = [
        ImageFont.truetype(
            os.path.join(os.path.dirname(__file__), "res/eng_92.ttf"), 126
        ),
        ImageFont.truetype(
            os.path.join(os.path.dirname(__file__), "res/zh_cn_92.ttf"), 95
        ),
    ]
    _bg = cv2.resize(
        cv2.imread(os.path.join(os.path.dirname(__file__), "res/yellow_bg.png")),
        (440, 140),
    )
    ...
    def _draw_char(self, ch):
        img = Image.new(
            "RGB", (45 if ch.isupper() or ch.isdigit() else 95, 140), (255, 255, 255)
        )
        draw = ImageDraw.Draw(img)
        draw.text(
            (0, -11 if ch.isupper() or ch.isdigit() else 3),
            ch,
            fill=(0, 0, 0),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "plate",
        help="license plate number (default: 京A12345)",
        type=str,
        nargs="?",
        default="京A12345",
    )
